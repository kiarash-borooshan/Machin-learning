...
plt.legend()
plt.title("NN-3hidden layer layer training Vs Validation loss \n"
...
plt.savefig(path + "zMapJPGExport-(BlnceDta)/" +
            "9-5- NN-3hidden layer training Vs Validation loss "
...
print("\n NN-3hidden-layers accuracy: %.2f \n" % Acc)

unique, counts = np.unique(yHtBlncDta, return_counts=True)
print("\n NN-3hidden-layersr count for (BlnceDta): \n",
...
print("\n NN-3hidden-layersr count Incorrect CsStdySmll for balance data: \n",
      dict(zip(unique, counts)))
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden-layersr layer for balance data duration: \n%.2f " % du, unit)
print("***************************** \n")
...
model.save("MyNN_3hiddenLyr_Model-DrpOut.h5")
...
M = load_model("MyNN_3hiddenLyr_Model-DrpOut.h5")
...
print("\n NN-3hidden CsStdySmll for balance data: \n",
      dict(zip(unique, counts)))
...
unique, counts = np.unique(predictBlncDtaInc, return_counts=True)
print("\n NN-3hidden Incorrect CsStdySmll for balance data: \n",
      dict(zip(unique, counts)))

Acc = 100 * np.mean(yHatCsStdySmll.reshape(1, -1) == yCsStdySmll1.reshape((1, -1)))
print("accuracy NN-3hidden for CsStdySmll:\n %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdySmllPath,
                                   "7-5_NN-3hidden_CsStdySmll(BlncDta)Drop-out=.2",
                                   "NN-3hidden_CsStdySmll(BlncDta)Drop-out=.2",
                                   predictBlncDtaInc)
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden layer for Case Study small balance data duration: \n %.2f " % du, unit)
print("***************************** \n")
...
print("\n NN-3hidden-layers_DrpOut CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))
...
unique, counts = np.unique(predictBlncDtaInc, return_counts=True)
print("\n NN-3hidden-layers_DrpOut Incorrect CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))

Acc = 100 * np.mean(yHatCsStdyBig.reshape(1, -1) == yCsStdyBig1.reshape((1, -1)))
print("accuracy NN-3hidden-layers_DrpOut for CsStdyBig\n: %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdyBigPath,
                                   "7-5_NN-3hidden-layers_DrpOut_CsStdyBig(BlncDta)",
                                   "NN-3hidden-layers_DrpOut_CsStdyBig(BlncDta)",
                                   predictBlncDtaInc)
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden-layers Drop out for Case Study Big balance data duration:\n %.2f " % du, unit)
print("***************************** \n")
