...
plt.savefig(path + "zMapJPGExport-(BlnceDta)/" +
            "9-2- NN-2hidden layer training Vs Validation loss "
            "(Adam, unit=100, 4 epoch=50)"
...
model.save("MyNN_2HiddenLyr_Model.h5")
...
M = load_model("MyNN_2HiddenLyr_Model.h5")
...
print("accuracy NN-2hidden for CsStdySmll: %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdySmllPath,
                                   "7-2_NN-2hidden_CsStdySmll(BlncDta)",
                                   "NN-2hidden_CsStdySmll(BlncDta)",
                                   predictBlncDtaInc)
...
print("\n NN-2hidden-layers CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))
...
unique, counts = np.unique(predictBlncDtaInc, return_counts=True)
print("\n NN-2hidden-layers Incorrect CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))

Acc = 100 * np.mean(yHatCsStdyBig.reshape(1, -1) == yCsStdyBig1.reshape((1, -1)))
print("accuracy NN-2hidden-layers for CsStdyBig: %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdyBigPath,
                                   "7-2_NN-2hidden-layers_CsStdyBig(BlncDta)",
                                   "NN-2hidden-layers_CsStdyBig(BlncDta)",
                                   predictBlncDtaInc)
