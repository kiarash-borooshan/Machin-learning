...
plt.savefig(path + "zMapJPGExport-(BlnceDta)/" +
            "9-4- NN-3hidden layer training Vs Validation loss "
            "(Adam, unit=100, 4 epoch=50)"
            "activation=relu, softmax .png", dpi=700)
...
print("\n NN-3hidden-layer accuracy: %.2f \n" % Acc)

unique, counts = np.unique(yHtBlncDta, return_counts=True)
print("\nNN-3hidden layer count for (BlnceDta): \n",
...
print("\n NN-3hidden count Incorrect CsStdySmll for balance data: \n",
      dict(zip(unique, counts)))
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden layer for balance data duration: %.2f " % du, unit)
print("***************************** \n")
...
model.save("MyNN_3hiddenLyr_Model.h5")
...
""" load Case studt Small data """
M = load_model("MyNN_3hiddenLyr_Model.h5")
...
print("accuracy NN-3hidden for CsStdySmll: %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdySmllPath,
                                   "7-4_NN-23hidden_CsStdySmll(BlncDta)",
                                   "NN-3hidden_CsStdySmll(BlncDta)",
                                   predictBlncDtaInc)
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden for Case Study small balance data duration: \n %.2f " % du, unit)
print("***************************** \n")
...
print("\n NN-3hidden-layers CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))
...
unique, counts = np.unique(predictBlncDtaInc, return_counts=True)
print("\n NN-3hidden-layers Incorrect CsStdyBig for balance data: \n",
      dict(zip(unique, counts)))

Acc = 100 * np.mean(yHatCsStdyBig.reshape(1, -1) == yCsStdyBig1.reshape((1, -1)))
print("accuracy NN-3hidden-layers for CsStdyBig: %.2f " % Acc)
...
zMyDl_utils.export_output_data_set(path + ClpCsStdyBigPath,
                                   "7-4_NN-3hiddenl-layers_CsStdyBig(BlncDta)",
                                   "NN-3hidden-layers_CsStdyBig(BlncDta)",
                                   predictBlncDtaInc)
...
if du > 60:
    unit = "Min"
    du = du / 60
print("NN-3hidden ayer for Case Study Big balance data duration: %.2f " % du, unit)
print("***************************** \n")
